#!/usr/bin/env python3
"""
Match 43213 addresses to Franklin County auditor parcel data.
...
ADDRESSES_FILE = '/Users/kevinpatel/Address/43213_project/data/addresses_43213.csv'
PARCELS_FILE = '/Users/kevinpatel/Address/43213_project/data/parcels_43213.csv'
OUTPUT_FILE = '/Users/kevinpatel/Address/43213_project/data/addresses_43213_enriched.csv'
...
def normalize_addr(addr_str):
    ...
    # Remove dash ranges like "454 - 456" -> "454"
...
def classify_occupancy(parcel):
    """Determine if owner-occupied or rental based on parcel data."""
    homstd = parcel.get('HOMSTD', '').strip()
    if homstd == 'Y':
        return 'OWNER_OCCUPIED'

    # Compare owner mailing address to property address
    ...
    if owner_addr and prop_addr:
        ...
        # If owner address matches property address, likely owner-occupied
        if prop_norm and prop_norm in owner_norm:
            return 'LIKELY_OWNER'

    # If owner address is different, it's likely a rental
...
def classify_land_use(code):
    """Human-readable land use classification."""
    land_use_map = {
...
def main():
    # Build parcel lookup by normalized address
    print("Loading parcel data...")
    parcel_lookup = {}
    # Also build a street-only lookup for apartment buildings
    street_lookup = {}
    ...
    with open(PARCELS_FILE, 'r') as f:
        ...
        for row in reader:
            ...
            # Primary key: normalized full address
            key = normalize_addr(staddr)
            if key not in parcel_lookup:
                parcel_lookup[key] = row

            # Handle address ranges like "4019 - 4021 ABBEY CT"
            # Index both numbers
            ...
            if range_match:
                ...
                # Also index all numbers in between (for duplexes)
                try:
            ...
            # Build street-level lookup for apartment matching
            # Key: "NUMBER STREET" without unit
    ...
    with open(ADDRESSES_FILE, 'r') as fin, open(OUTPUT_FILE, 'w', newline='') as fout:
        ...
        for row in reader:
            ...
            # If no match and has a unit, try matching without unit to parent building
            if not parcel and row.get('unit', ''):
                parcel = street_lookup.get(addr_key)

            # Try without directional prefix variations
            if not parcel:
                # Try adding S/N/E/W prefix if not present
            ...
            # Try removing S/N/E/W prefix if present
            if not parcel:
    ...
    print(f"\n=== Results ===")
    print(f"Total addresses: {matched + unmatched}")
    print(f"Matched to parcels: {matched} ({matched/(matched+unmatched)*100:.1f}%)")
